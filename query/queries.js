...
db.restaurants.find( { cuisine: { $ne: "American" }, borough: { $ne: "Brooklyn" }, "grades.grade": { $eq: "A" } }, { _id: 0 } ).sort({cuisine:-1})
...
db.restaurants.find({ borough: "Bronx", $or: [ { cuisine: { $eq: "American" } }, { cuisine: { $eq: "Chinese" } } ] }, { _id: 0 } )
...
db.restaurants.find( { name: { $regex: /mon/i} }, { _id: 0, name:1, borough:1, "location.coordinates":1, cuisine:1} )
